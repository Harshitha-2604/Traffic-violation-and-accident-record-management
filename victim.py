...
class Traffic:
    def __init__(self,root):
        ...
        #upper frame
        upper_frame=LabelFrame(Main_frame,bd=2,relief=RIDGE,text="VICTIM INFORMATION",font=('times new roman',11,'bold'),fg='red',bg='white')
        upper_frame.place(x=10,y=10,width=1480,height=270)

        # Label entry

        #victim id
        Caseid=Label(upper_frame,text='Victim ID:',font=('arial',11,'bold'),bg='white')
        Caseid.grid(row=0,column=0,padx=2,sticky=W)

        casentry=ttk.Entry(upper_frame,textvariable=self.var_victim_id,width=22,font=('arial',11,'bold'))
        ...
        vechentry=ttk.Entry(upper_frame,textvariable=self.var_address,width=22,font=('arial',11,'bold'))
        vechentry.grid(row=2,column=1,padx=2,pady=7)

        #accident id
        dateofcrime=Label(upper_frame,text='Accident ID:',font=('arial',12,'bold'),bg='white')
        dateofcrime.grid(row=2 ,column=2,padx=2,sticky=W,pady=7)

        dateofvio=ttk.Entry(upper_frame,textvariable=self.var_acc_id,width=22,font=('arial',11,'bold'))
        dateofvio.grid(row=2,column=3,sticky=W,padx=2,pady=7)

       # Gender Label
        gender = Label(upper_frame, text='Gender:', font=('arial', 12, 'bold'), bg='white')
        gender.grid(row=3, column=0, padx=2, sticky=W, pady=7)

        # Radio Button Frame - Gender
        radio_frame_gender = Frame(upper_frame, bd=2, relief=RIDGE, bg='White')
        radio_frame_gender.grid(row=3, column=1, padx=2, pady=7, sticky=W)
        ...
        Button_frame.place(x=5,y=200,width=620,heigh=45)
        ...
        self.vio_table.heading('1',text='Victim ID')
        ...
        self.vio_table.heading('7',text='Accident ID')
    ...
    def add_data(self):
        if self.var_victim_id=="":
            ...
            try:
                ...
                my_cursor.execute('''
                    INSERT INTO victims (VictimID,FirstName,LastName,DateOfBirth,Gender,Address,AccidentID)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                    ''', (
                            self.var_victim_id.get(),
                            self.var_fn.get(),
                            self.var_ln.get(),
                            self.var_dob.get(),
                            self.var_gender.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo('Success', 'Victim record has been added')
    ...
    def fetch_data(self):
        ...
        my_cursor=conn.cursor()
        my_cursor.execute('select * from victims')
        ...
        if self.var_acc_id.get()=="":
            ...
            try:
                ...
                if update>0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute('update victims set FirstName=%s,LastName=%s,DateOfBirth=%s,Gender=%s,Address=%s,AccidentID=%s where VictimID=%s',(     
                                                                                                    self.var_fn.get(),
                                                                                                    self.var_ln.get(),
                                                                                                    self.var_dob.get(),
                                                                                                    self.var_gender.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo('Success','Victim Information has been successfully updated')
            except Exception as es:
                messagebox.showerror('Error',f'Due to{str(es)}')

    #delete
    def delete_data(self):
        if self.var_victim_id.get()=="":
            messagebox.showerror('Error','All the fields are required')
        else:
            try:
                ...
                if delete>0:
                    ...
                    my_cursor = conn.cursor()
                    sql='delete from Victims where VictimID=%s'
                    value=(self.var_victim_id.get(),)
